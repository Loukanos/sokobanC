/**
    ...
    versão: 0.6 (Lucas)
    Notas: O objetivo dessa versão é, principalmente, remover as variáveis estáticas de forma a manter o funcionamento.
*/
#include<iostream>
#include<windows.h>
#include<conio.h>
...
void menu(int m[8][9],int m2[8][9], int m3[8][9], int movimento, bool continua, int escolha);//protótipo da função menu para poder realizar a chamada
        ...
        for(int i=0;i<8;i++){//laço para impressão da matriz e elementos importantes
            for(int j=0;j<9;j++){
                if(i==x && j==y){
                    ...
                    switch (m[i][j]){
                        case 0:if ((i==a && j ==b) || (i==c && j ==d) || (i ==e && j==f)|| (i==g && j==h) ||(i==k && j==l))//imprime objetivos
                        {
                            cout<<char(158);
                        }
                        else{
                         cout<<" "; //imprime espaço vazio
                        }
                        break;
...
void voltarMenu(int m[8][9],int m2[8][9], int m3[8][9],bool &game, int movimento, bool continua, int escolha){
...
if (GetAsyncKeyState('1') & 0x8000) {
  system("cls");
  menu(m, m2, m3, movimento, continua, escolha);
  game = false;
}
...
if(escolha == 1){
    ...
    while(game == true){
        ...
        voltarMenu(m,m2,m3,game, movimentos, continua, escolha);
...
void sobre(int m[8][9],int m2[8][9], int m3[8][9], int movimento, bool continua, int escolha){//submenu "sobre"
...
void menu(int m[8][9],int m2[8][9], int m3[8][9], int movimento, bool continua, int escolha){//menu principal
   ...
   while (lacoMenu == 0){
    ...
    if ( _kbhit() ){
        ...
        case 51://3-Sobre
            system("cls");
            sobre(m,m2,m3, movimento, continua, escolha);
            lacoMenu++;
            break;
        case 52://4-fim
            lacoMenu++;
...
//Principamente, remover variáveis estáticas----->A maioria foi removida
//Nas coordenadas, professor sugeriu usar vetores------>não funcionou!
//para as matrizes, ele sugeriu fazer uma função separada para a declaração de mapas----->aplicado
//no mais, ele disse que o tamanho das funções está bom
//não é necessário explicar o código do cursor
//melhorias concluidas, falta apenas a ultima variavel estatica. Removida na versao 07.